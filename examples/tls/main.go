...
func main() {
	...
	fmt.Println(aurora.Magenta("MQTT Server initializing..."), aurora.Cyan("TLS/SSL"))
	...
	ws := listener.NewWebsocketWithTLS("ws1", ":1882", &mqtt.AuthAllow{}, cfg)
	err = server.AddListener(ws)
	if err != nil {
		log.Fatal(err)
	}

	stats := listener.NewHTTPStatsWithTLS("stats", ":8080", cfg)
